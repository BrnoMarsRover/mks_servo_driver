...
class CalibrationResult(Enum):
    """
    Calibration needed for the close modes
    """
    Unknown = 500
    Calibrating = 0
    CalibratedSuccess = 1
...
class WorkMode(Enum):
    """
    Cr means pulse interface, Sr serial interface (CAN). Open means the motor runs without the encoder,
    Close runs with the encoder (requires calibration). vFoc mode adapts the current to the actual needs up to set
    Max current (Ma). Other modes use Max current (Ma) as a working current.
    """
    CrOpen = 0
    CrClose = 1
    CrvFoc = 2
